...
function looksLikeWeakName(n) {
  ...
  return (
    ...
    t.toLowerCase() === (t.toLowerCase().replace(/\s/g,'')) ||
    ...
    try {
      ...
      py.on('close', (code) => {
        ...
        try {
          ...
          settle({ ok: false, error: err || `Invalid JSON from Python extractor: ${trimmedOut.substring(0, 120)}` });
        }
      });
      ...
      try {
        ...
        settle({ ok: false, error: `Python stdin write failed: ${writeErr.message}` });
      }
...
const CDN_HOSTING_DOMAINS = [
  ...
  'railway.app','fly.dev','deno.dev','github.io','gitlab.io','bitbucket.io',
  'shopify.com','myshopify.com','squarespace.com','wixsite.com','weebly.com',
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      try {
        ...
        const html = typeof fb.data === 'string' ? fb.data : '';
        return { result: r, html };
...
function analyzeContent(html, domain, finalUrl) {
  ...
  const isSPA = (
    ...
    /<noscript[^>]*>[\s\S]{20,}<\/noscript>/i.test(html)
  );
  ...
  async function analyzeSingleDomain(rawDomain) {
    ...
    const work = (async () => {
      try {
        ...
        const contentAnalysis = analyzeContent(html, domain, httpStatus.finalUrl);
        const isWAFBlock = httpStatus.isUp && httpStatus.statusCode === 403 && (
          html.length === 0 || (html.length < 200 && !/<html/i.test(html) &&
            /host not allowed|access denied|forbidden|blocked|not authorized/i.test(html)));
...
// ═══════════════════════════════════════════════════════════════
// BUSINESS INFO EXTRACTION ENGINE
// ═══════════════════════════════════════════════════════════════
...
function cleanBusinessName(rawTitle, ogSiteName, schemaName, domain, footerName) {
  ...
  const cleanName = (s) => {
    if (!s) return s;
    let cleaned = s.replace(/[•·|]+$/, '').replace(/^[•·|]+/, '').replace(/\s+/g, ' ').trim();
  ...
  const expanded = words.flatMap(w => {
    ...
    if (leadNumMatch) {
      const num = leadNumMatch[1]; const rest = leadNumMatch[2];
      if (rest.length <= 2) return [num + rest.toUpperCase()];
      const prefix1 = rest.match(/^([a-z])([a-z]{3,})$/i);
      const prefix2 = rest.match(/^([a-z]{2})([a-z]{3,})$/i);
      const prefixMatch = prefix1 || prefix2;
      if (prefixMatch) {
        const prefix = prefixMatch[1]; const mainWord = prefixMatch[2];
      ...
      return [num, ...restSplit.map(s => s.charAt(0).toUpperCase() + s.slice(1))];
    }
...
function extractBusinessNameCheerio(html) {
  ...
  if (twitterTitle) {
    ...
    if (name && name.length > 2 && name.length < 80 && !isBoilerplateName(name)) {
      return { name, source: 'twitter:title', confidence: 80 };
    }
  }
  ...
  if (appName && appName.trim().length > 1) {
    ...
    if (name && name.length > 2 && name.length < 80 && !isBoilerplateName(name)) {
      return { name, source: 'application-name', confidence: 85 };
    }
  }
  ...
  const logoSelectors = [
    ...
    'img[class*="site-logo"]','img[class*="header-logo"]','img[class*="custom-logo"]',
  ...
  for (const sel of logoSelectors) {
    ...
    $(sel).each((_, el) => {
      ...
      if (alt.trim().length > 2) {
        const name = cleanBizName(alt.trim().replace(/\s*logo\s*/gi, '').trim());
        if (name && name.length > 2 && !/^(logo|image|img|banner|header|icon|photo|pic)$/i.test(name)) {
          found = name;
        }
      }
    });
...
function extractEnhancedBusinessName(html) {
  ...
  const appName = $('meta[name="application-name"]').attr('content');
  if (appName && appName.trim().length > 1) candidates.push({ name: appName.trim(), source: 'application-name', priority: 7 });

  const appleTitle = $('meta[name="apple-mobile-web-app-title"]').attr('content');
  if (appleTitle && appleTitle.trim().length > 1) candidates.push({ name: appleTitle.trim(), source: 'apple-title', priority: 7 });

  const twitterTitle = $('meta[name="twitter:title"], meta[property="twitter:title"]').attr('content');
  if (twitterTitle && twitterTitle.trim().length > 1) candidates.push({ name: twitterTitle.trim(), source: 'twitter-title', priority: 5 });

  $('[itemprop="name"]').not('script [itemprop="name"]').each((_, el) => {
    const name = $(el).text().trim();
    if (name.length > 2 && !isBoilerplateName(name)) candidates.push({ name, source: 'itemprop-name', priority: 9 });
  });
  ...
  if (copyrightMatch) candidates.push({ name: copyrightMatch[1].trim(), source: 'copyright-legal', priority: 10 });
  ...
  function processItem(item) {
    ...
    if (addrSource && !result.address) {
      ...
      if (typeof a === 'string') result.address = { raw: a };
        ...
        state: a.addressRegion || null, zip: a.postalCode ? String(a.postalCode) : null, country: a.addressCountry || null
      };
    }
...
function extractSocialSignals(html, schema) {
  ...
  for (const [key, val] of Object.entries(signals)) {
    ...
    result.push({ platform: platform?.label || val.platform || key, url: val.url, icon: platform?.icon || key });
  }
  return result;
}
  ...
  for (const [key, platform] of Object.entries(SOCIAL_PLATFORMS)) {
    if (urlLower.includes(platform.domain)) {
      ...
      try {
        ...
        if (key === 'googlebiz') { if (!signals[normalizedKey]) signals[normalizedKey] = { platform: platform.label, url, source: 'link' }; return; }
        if (key === 'whatsapp' || key === 'telegram') { if (!signals[normalizedKey]) signals[normalizedKey] = { platform: platform.label, url, source: 'link' }; return; }
...
function parseUSAddress(rawAddress) {
  ...
  if (rawLines.length > 1) {
    ...
    if (streetLine && cszLine && streetLine !== cszLine) {
      ...
      const preferredLine = filteredLines.find(line => cityStateZipPattern.test(line)) || streetLine || filteredLines[0];
  ...
  let addr = decodeHTML(String(rawAddress))
    ...
    .replace(/,\s*(?:US|USA|United\s+States)\s*$/i, '').trim();

  addr = addr.replace(/^\s*CCB\s*#?\s*\d+\s*,?\s*/i, '').replace(/^\s*(?:sign\s*up|subscribe|newsletter)\b[^\dP]{0,60}/i, '').trim();
  ...
  if (streetStart > 0 && !/^(?:P\.?\s*O\.?|Post\s+Office)\s*Box\b/i.test(addr)) addr = addr.substring(streetStart).trim();
  ...
  if (poBoxFull) {
    ...
    result.city = poBoxFull[2].trim(); result.state = poBoxFull[3].toUpperCase(); result.zip = poBoxFull[4];
    return result;
  }

  const cleanToken = (v = '') => String(v).replace(/^\s*,|,\s*$/g, '').replace(/\s+/g, ' ').trim();
  const stripTrailingCityFromStreet = (street, city) => {
    const s = cleanToken(street); const c = cleanToken(city);
    if (!s || !c) return s;
  ...
  if (strictComma) {
    ...
    if (/^\d{5}$/.test(result.city)) { result.zip = result.zip || result.city; result.city = ''; }
  ...
  if (!tail) tail = addr.match(/,\s*([A-Za-z][A-Za-z\s.'-]{1,40})\s+([A-Za-z]{2})\s*(\d{5}(?:-\d{4})?)?\b/);
  if (tail) { result.city = tail[1].trim(); result.state = tail[2].toUpperCase(); if (tail[3]) result.zip = tail[3]; }
  ...
  if (parts.length >= 3) {
    ...
    if (/^\s*[A-Za-z]{2}\s*\d{5}/i.test(lastPart) || /^\s*[A-Za-z]{2}\s*$/i.test(lastPart)) {
      ...
      else result.city = cityPart;
    }
    if (parts.length >= 4) result.street = parts.slice(0, parts.length - 2).join(', ');
    ...
    if (hasStreetWords) {
      ...
      result.street = parts[0]; result.city = parts[1];
    }
  } else {
  ...
  if (result.street) {
    result.street = result.street.replace(/[\]\[{}<>]+/g, ' ').replace(/[•·]+/g, ' ').replace(/\s+/g, ' ').replace(/[,;:\-]+$/, '').trim();
  }
  ...
  if (/^\d{5}$/.test(result.city)) { if (!result.zip) result.zip = result.city; result.city = ''; }
  if (result.zip) { const zip5 = String(result.zip).match(/\b(\d{5})\b/); result.zip = zip5 ? zip5[1] : ''; if (result.zip && result.city === result.zip) result.city = ''; }
  result.street = stripTrailingCityFromStreet(result.street, result.city);
...
function detectCountry(html, domain, schemaAddress, phones) {
  ...
  const bodyText = html.replace(/<script[\s\S]*?<\/script>/gi, '').replace(/<style[\s\S]*?<\/style>/gi, '').replace(/<[^>]+>/g, ' ').replace(/&[a-z]+;/gi, ' ').replace(/\s+/g, ' ');
  ...
  const hasUSZip = (usZipWithComma && US_STATE_SET.has(usZipWithComma[1])) || (usZipNoComma && US_STATE_SET.has(usZipNoComma[2]));
  if (hasUSZip) {
  ...
  const strictUKpat = /(?:,\s*|[Ll]ondon\s+|[Mm]anchester\s+|[Bb]irmingham\s+|[Ll]iverpool\s+|United Kingdom\s+|UK\s+)[A-Z]{1,2}\d[A-Z\d]?\s\d[A-Z]{2}\b/;
...
async function getDomainAge(domain) {
  ...
  const RDAP_ENDPOINTS = {
    ...
    biz:'https://rdap.nic.biz/domain/',io:'https://rdap.nic.io/domain/',
...
function buildBusinessStrengthSignals({ businessName, phones, emails, address, schema, metaDescription, socialSignals }) {
  ...
  const points = (hasBusinessName?25:0) + (hasPhone?18:0) + (hasEmail?17:0) + (hasStreetAddress?13:0) + (hasLocality?5:0) + (hasSchemaName?5:0) + (hasMetaDescription?5:0) + (hasSocialPresence?Math.min(12,socialCount*4):0);
  const score = Math.max(0, Math.min(100, points));
  let confidence = 'low';
  if (score >= 75) confidence = 'high';
  else if (score >= 45) confidence = 'medium';
  return { score, confidence, signals: { hasBusinessName, hasPhone, hasEmail, hasStreetAddress, hasLocality, hasSchemaName, hasMetaDescription, hasSocialPresence, socialCount } };
}
...
async function extractBusinessInfo(html, domain) {
  ...
  if (schema.address) {
    if (schema.address.street || schema.address.city) {
      address = { street:schema.address.street||'', city:schema.address.city||'', state:schema.address.state||'', zip:schema.address.zip?String(schema.address.zip):'' };
  ...
  domRaw.split(/[-_.]/).forEach(part => {
    const subparts = part.replace(/(\d)([a-zA-Z])/g, '$1 $2').replace(/([a-z])([A-Z])/g, '$1 $2').split(/\s+/);
    subparts.forEach(sp => {
      const clean = sp.replace(/\d+/g, '');
      if (clean.length > 2) domWords.push(clean.toLowerCase());
      if (sp.length > 2) domWords.push(sp.toLowerCase());
  ...
  if (!businessName || businessName.trim().length === 0) businessName = domain;
  ...
  if (!businessName || businessName.length < 5 || businessName.toLowerCase() === domain.toLowerCase() || nameIsSlug || currentNameOverlap === 0) {
    ...
    while ((nm = nameRegex.exec(bodyText)) !== null) {
      ...
      if (candidate.length > 5 && candidate.length < 80 && !isBoilerplateName(candidate)) {
        ...
        if (score > bestCandidateScore) { nameCandidate = candidate; bestCandidateScore = score; }
      }
    }
...
async function fetchContactPage(domain, homepageHtml) {
  try {
    const contactSlugs = [
      ...
      '/contact-information', '/our-team', '/company',
    ...
    for (let i = 0; i < toTry.length; i += 3) {
      ...
      try {
        const result = await Promise.any(batch.map(slug => trySlug(slug).catch(e => Promise.reject(e))));
        return result;
      } catch {}
    }
  } catch {}
...
app.post('/api/extract-business', async (req, res) => {
  ...
  try {
    ...
    const isWAFBlock = httpStatus.isUp && httpStatus.statusCode === 403 && (
      html.length === 0 ||
      (html.length < 200 && !/<html/i.test(html) && /host not allowed|access denied|forbidden|blocked|not authorized/i.test(html)));
    ...
    const emptyBusiness = { businessName:'', phones:[], emails:[], address:{street:'',city:'',state:'',zip:''}, country:{code:'UNKNOWN',name:'Unknown',confidence:'none'}, metaDescription:null, businessType:null, socialSignals:[], strength:{score:0,confidence:'low',signals:{hasBusinessName:false,hasPhone:false,hasEmail:false,hasStreetAddress:false,hasLocality:false,hasSchemaName:false,hasMetaDescription:false,hasSocialPresence:false,socialCount:0}} };
    ...
    if (needsContactPage && contactHtml) {
      ...
      console.log(`  [CONTACT PAGE] merged data from contact subpage`);
    }
    ...
    try {
      ...
      bizResult = {
        ...
        business: {
          ...
          country: business?.country || { code:'UNKNOWN', name:'Unknown', confidence:'none' },
          metaDescription: business?.metaDescription || null,
          businessType: business?.businessType || null,
          socialSignals: business?.socialSignals || [],
          strength: business?.strength || { score:0, confidence:'low' }
        }
      };
    }
    res.json(bizResult);
...
app.get('/api/domain-age', async (req, res) => {
  ...
  try {
    ...
    res.json({ domain, ageText:result.ageText||null, createdDate:result.createdDate||null, ageInDays:result.ageInDays||null, registrar:result.registrar||null, error:result.error||null });
  } catch (e) {
